...
A Chrome/Edge Manifest V3 browser extension ("JobBot" internally, branded **AutoApplier** on the site) that automates job applications on **LinkedIn (Easy Apply), Indeed, Naukri, Naukri Gulf, and Bayt** (the core agent), plus separate isolated add-ons for **Internshala** bulk-apply and **Reddit** post scheduling. The repo also deploys to Vercel as a static site + zero-dependency serverless backend, with a **license-key based access model** and a **programmatic SEO surface** (~47 generated marketing pages) on top.
...
- **Repackage the zip after ANY change under `extension/`:** run `node scripts/build-extension-zip.mjs` (one-time setup: `npm --prefix .build-tools install`). This produces an **obfuscated** `jobbot-extension.zip` (identifier mangling + string/selector/URL encoding) so a customer with the zip can't easily read or clone the code — while the repo's `extension/` source stays readable (the owner develops from it and "Load unpacked"s it directly; the **admin/owner is the exception**). Do NOT ship a plain `zip -rq jobbot-extension.zip extension` for downloads — that leaks the source. The committed zip is what the landing page serves; it must stay in sync with the source version. Obfuscation is deliberately conservative (no control-flow flattening / self-defending / debug traps) so the agent's behaviour is unchanged; after any extension change, test the built zip on LinkedIn/Indeed/Naukri/Bayt before relying on it.
...
- **One-command release (preferred):** `scripts/release.sh <patch|minor|major|X.Y.Z> [-n "notes"] [--commit] [--tag] [--push]` bumps `manifest.json` + `version.json` together, rebuilds `jobbot-extension.zip`, and runs the same preflight CI runs — so the three-way sync can't drift. `scripts/release.sh --check` runs that preflight against the current tree without changing anything (good pre-push gate). It refuses non-increasing versions and only ever touches the two version files + the zip.
- **Syntax-check before committing** (there are no tests): `node -e "new Function(require('fs').readFileSync('extension/content.js','utf8'))"` for the IIFE content scripts (also `linkedin_autoapply.js`, `internshala_autoapply.js`, `reddit_scheduler.js`), `node --check extension/background.js extension/popup.js`, and `node --input-type=module --check < api/<file>.js` for the ESM serverless functions.
- **Regenerate SEO pages after editing their generator:** `node scripts/build-seo.mjs` writes all static SEO landing pages (platform, role, city, competitor-alternative, blog), `sitemap.xml`, and `robots.txt` into the repo root. These are pure static HTML that add NO serverless functions and touch NO app logic. **Never hand-edit the generated `*.html`/`sitemap.xml`** — edit `scripts/build-seo.mjs` and rebuild.
...
### Geo pricing (important)
`api/_lib.js` charges each visitor in their own region's currency. `countryFromReq(req)` trusts the Vercel edge header `x-vercel-ip-country`; `regionForCountry(cc)` groups it; `priceForPlan(plan, cc)` resolves **exact country → region group → `DEFAULT` → legacy INR `price_paise`**. Per-country/region prices live in a `plans.prices` **jsonb** column (admin sets them in `admin.html`). `CURRENCY_SYMBOL` is an allowlist. Public endpoints (`api/plans.js`, `api/checkout.js`) return only the resolved `{currency, symbol, amount}` for the visitor's country — never the full price map (privacy). Monthly plans need a per-currency Razorpay subscription plan id; coupons are INR-only. Several code paths degrade gracefully if a migration (`plans.prices`, `purchases.currency/region`, `users.name/phone`, `demo_requests`) hasn't been run — the code cannot do DDL, so those are manual Supabase SQL steps.
...
`index.html` (AutoApplier SPA: email login/signup → dashboard with activation-key card, download, referral, change-password, geo-priced plans; embedded admin panel for admin logins), `crm.html` (job tracker; login by license key OR email/password), `checkout.html` (plans + Razorpay + one-step account creation; reads `?ref=CODE`), `admin.html` (full console: License Keys, Plans w/ per-region prices, Users & Access + bulk-delete, Referral Codes, Purchases, Demos), `reset.html` (password reset link target). The **generated SEO pages** (`*-auto-apply.html`, `auto-apply-*.html`, `*-alternative.html`, `blog/`) + `sitemap.xml`/`robots.txt` are produced by `scripts/build-seo.mjs` — edit the generator, not the output. Both `index.html` and every SEO page share a tripmore-style mega footer; `assets/seo.css` styles the SEO pages.
...
## Isolated add-on features (hard isolation rule)

Some features live in their **own separate content-script file** and are deliberately walled off from `content.js`/`background.js`/`popup.js`. Each has its own singleton guard, its own DOM-class namespace, its own `chrome.storage.local` keys, and **must not import, call, read, or write any state of the core agent**. They are registered additively in `manifest.json` (own `content_scripts` entry + host permission) and appear in the obfuscation allowlist in `scripts/build-extension-zip.mjs`. When the user says "new isolated feature, do not touch existing code," build it this way:

- **`extension/linkedin_autoapply.js`** — LinkedIn mass-apply (guard `__jobbotLinkedInMassApply*`). *Not* the same as the LOCKED `LinkedInAgent` in `content.js`; it has its own autofill (`APPLY_PROFILE` mapped from `jobbot_profile`, read-only, refreshed via `chrome.storage.onChanged`). Editing this file does NOT touch the locked agent.
- **`extension/internshala_autoapply.js`** — Internshala bulk-apply (guard `__jobbotInternshalaAutoApplyV1`, `jbia-` namespace, `jobbot_internshala_applied`). Reads `jobbot_profile` + `GET_LICENSE` only.
- **`extension/reddit_scheduler.js`** — Reddit CSV post scheduler (guard `__jobbotRedditSchedulerV1`, `jbrs-` namespace, `jbrs_*` keys, `chrome.storage.local` only, no runtime messaging). Reddit's composer is open-shadow-DOM + a Lexical editor, so field access uses a shadow-piercing `deepQuery`, body is filled via `execCommand('insertText')`, and the run survives page reloads between posts via stored queue+index (auto-resume on each submit-page load).
